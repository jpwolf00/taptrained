  ...
  useEffect(() => {
    fetch("/api/admin/settings")
      ...
      .then((d) => {
        ...
        setVenueName(d.name ?? "");
        if (d.invite_code) {
          setJoinLink(`${window.location.origin}/join?code=${d.invite_code}`);
        }
        setLoading(false);
      });
  }, []);
      ...
      <section className="mb-8 rounded-xl border border-border bg-surface p-4">
        ...
        <div className="mt-4 border-t border-border pt-4">
          <p className="text-xs text-muted">
            Share this link with your staff — the invite code is filled in for them.
          </p>
          <div className="mt-2 flex items-center gap-2">
            <code className="min-w-0 flex-1 truncate rounded-lg border border-border bg-surface-2 px-3 py-2 text-xs text-foreground/80">
              {joinLink || "…"}
            </code>
            <button
              type="button"
              ...
              disabled={!joinLink}
              className="shrink-0 rounded-lg bg-amber px-3 py-2 text-xs font-semibold text-[#1a1209] transition hover:bg-amber-deep disabled:opacity-50"
            >
